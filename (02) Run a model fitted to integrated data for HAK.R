###############################################################################################################################
##
##  (02) This script runs a model fitted to integrated data (i.e., TRAWL BT data + COD BT data) for HAK
##  
###############################################################################################################################

######## Set the output directory
if ( !dir.exists( paste0( DIR$Output, "\\HAK_Integrated_data" ) ) ) {
	dir.create( paste0( DIR$Output, "\\HAK_Integrated_data" ) )
} 
OutputDir <- paste0( DIR$Output, "\\HAK_Integrated_data" )
setwd( OutputDir )
...
program_ref <- 1
Q_ik <- as.matrix( cbind( ThorsonUtilities::vector_to_design_matrix( Sp_data$Monitoring_program )[,-program_ref] ) )
